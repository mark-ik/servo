...
pub(crate) fn analyze_text<B: Brush>(
    ...
    // which normally takes the fast left-to-right path.
        ...
        .fold(false, |is_mandatory_linebreak, (boundary, ch)| {
            ...
            lcx.info.push((
                CharInfo::new(
                    ...
                    contributes_to_shaping(ch, general_category, script),
                    force_normalize,
                ),
...
/// All characters require nominal glyph coverage except:
/// - Control characters
/// - Format characters, unless they use the "Inherited" script
/// - Join controls, which still reach the shaper but must not force fallback
#[inline(always)]
pub(crate) fn contributes_to_shaping(
    ch: char,
